...
LATENCIES=(0.300 0.325 0.350 0.375 0.400)
...
make_random_dirs () {
  mkdir -p logs/random/{splits,system}
}
...
for lat in "${LATENCIES[@]}"; do
  ...
  echo "=============================="
  echo "[$(ts)] Random run: MAX_INFERENCE_LATENCY=$lat"
  echo "=============================="
  ...
  set_ini_value "ALGORITHM" "SPLIT_ALGORITHM" "1"
  ...
  cp "$CONFIG_FILE" "logs/config_random_lat_${safe_lat}.ini"
  run_log="logs/output_random_lat_${safe_lat}.log"

  echo "[$(ts)] Starting python (log: $run_log)"
  CUDA_VISIBLE_DEVICES="$GPU_ID" python main.py 2>&1 | tee "$run_log"

  dest="logs/random_lat_${safe_lat}"
  if [[ -e "$dest" ]]; then
  ...
  echo "[$(ts)] Finished random run (lat=$lat). Saved to $dest"
  echo
done

echo "[$(ts)] All random latency runs completed."