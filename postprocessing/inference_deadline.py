...
from postprocessing.network_throughput import return_ci_95, return_ci_90
...
def plot_kpis_vs_inference_deadline(df_all_inference_time, df_all_ue_energy_comp, df_all_ue_energy_comm,
    ...
    df_inference_time_opt_list = df_all_inference_time[2]   # opt
    df_ue_energy_comp_opt_list = df_all_ue_energy_comp[2]   # opt
    df_ue_energy_comm_opt_list = df_all_ue_energy_comm[2]   # opt
    df_top1_opt_list = df_all_top1[2]                       # opt
    df_inference_time_random_list = df_all_inference_time[3]    # random
    df_ue_energy_comp_random_list = df_all_ue_energy_comp[3]    # random
    df_ue_energy_comm_random_list = df_all_ue_energy_comm[3]    # random
    df_top1_random_list = df_all_top1[3]                        # random
    ...
    df_top1_fixed_list = df_all_top1[4]                         # fixed
    ...
    df_top1_hr_list = df_all_top1[5]  # heuristic
    ...
    plt.errorbar(x=np.array(inference_deadline_list) - 0.006, y=ue_energy_comp_mean_per_deadline_opt,
                 yerr=ue_energy_comp_ci_per_deadline_opt, color="#072140",  # line + marker color
                 ...
                 linestyle='-',
                 fmt='o', label='opt')
    plt.errorbar(x=np.array(inference_deadline_list) - 0.003, y=ue_energy_comp_mean_per_deadline_ddqn,
                 yerr=ue_energy_comp_ci_per_deadline_ddqn, color="#114584",  # line + marker color
    ...
    plt.errorbar(x=np.array(inference_deadline_list) + 0.006, y=ue_energy_comp_mean_per_deadline_fixed,
                 yerr=ue_energy_comp_ci_per_deadline_fixed, color="#8F81EA",  # line + marker color
...
def main():
    ...
    n_episodes_to_train = {'rl/ddqn': 4000, 'rl/a2c': 3000}
    ...
    df_all_top1 = [df_top1_list_ddqn, df_top1_list_a2c, df_top1_list_opt, df_top1_list_random, df_top1_list_fixed,
                   df_top1_list_hr]
