...
def read_kpis_from_files(folder, kpi_type, episode_count, max_throughput):
    ...
    #else:
    file = 'logs/{}/comparison/throughput_{}/system/{}_{}.csv'.format(folder, max_throughput, kpi_type, episode_count)
    data_timestep = []
    data_kpi = []
...
def plot_kpis_vs_max_throughput(df_all_inference_time, df_all_ue_energy_comp, df_all_ue_energy_comm,
    ...
    plt.grid()
    plt.legend()
    plt.savefig('results/comm_energy_vs_max_throughput.png')
    plt.savefig('results/comm_energy_vs_max_throughput.svg')
    plt.show()
...
def main():
    ...
    for max_throughput in max_network_throughput_list:
        ...
        df_inference_time, df_ue_energy_comp, df_ue_energy_comm, _, _ = parse_kpis('optimum', 1, max_throughput)
        ...
        df_inference_time, df_ue_energy_comp, df_ue_energy_comm, _, _ = parse_kpis('random', 10, max_throughput)
        ...
        df_inference_time, df_ue_energy_comp, df_ue_energy_comm, _, _ = parse_kpis('fixed', 1, max_throughput)
